...
const buildOverviewMessage = async (userId: string, name?: string | null) => {
  ...
  const message = [
    ...
    `🧠 Score: ${score}/100`,
    `🖼️ Arti gambar: ${animal.name} = ${animal.description}`,
    ...
    '',
    'Makna level ini:',
    `- Nama hewan/gambar: ${animal.name}`,
    `- Gambaran kondisi: ${animal.description}`,
    `- Fokus perbaikan: ${animal.tip}`,
    '',
    ...
    `3. Jaga progres level supaya naik dari ${animal.name} ke level berikutnya.`,
  ].filter(Boolean).join('\n');
